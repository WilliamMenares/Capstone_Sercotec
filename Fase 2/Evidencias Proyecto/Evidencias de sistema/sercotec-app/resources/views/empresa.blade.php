@if (auth()->user()->rol == 0)
    ...
    @section('content')
        ...
        <div class="crud">
            ...
            <div class="botones-empresa">
                <div class="create-nuevo">
                    <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#createModal">Crear
                        una
                        Empresa</button>
                </div>
                ...
                <form id="import-form" enctype="multipart/form-data">
                    ...
                    <button type="button" onclick="startImport()" class="boton-importar btn btn-secondary ml-2">
                        Importar Excel
                    </button>
                </form>
            </div>
                ...
                <div class="mobile-table-body" id="mobileTableBody">
                    @foreach ($empresas as $emp)
                        <div class="mobile-table-row" data-codigo="{{ $emp->codigo }}" data-rut="{{ $emp->rut }}"
                            data-nombre="{{ $emp->nombre }}" data-email="{{ $emp->email }}"
        ...
        @foreach ($empresas as $emp)
            <div class="modal fade" id="deleteModal{{ $emp->id }}" data-bs-backdrop="static" data-bs-keyboard="false"
            ...
            <div class="modal fade" id="editModal{{ $emp->id }}" data-bs-backdrop="static" data-bs-keyboard="false"
                            ...
                            <form action="{{ route('empresa.update', $emp->id) }}" class="formcru" method="POST">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control bg-dark text-light rut-vali" name="rut"
                                        placeholder="12345678-9" value="{{ $emp->rut }}" autocomplete="off"
                                        required>
                                </div>
                                <div class="mb-3">
                                    <label for="nombre" class="form-label">Nombre:</label>
                                    <input type="text" class="form-control bg-dark text-light" name="nombre"
                                        placeholder="nombre" value="{{ $emp->nombre }}" autocomplete="off" required>
                                </div>
                                <div class="mb-3">
                                    <label for="email" class="form-label">Email:</label>
                                    <input type="email" class="form-control bg-dark text-light" name="email"
                                        placeholder="ejemplo@ejemplo.com" value="{{ $emp->email }}" autocomplete="off"
                                        required>
                                </div>
                                <div class="mb-3">
                                    <label for="contacto" class="form-label">Contacto:</label>
                                    <input type="text" class="form-control bg-dark text-light" name="contacto"
                                        placeholder="contacto" value="{{ $emp->contacto }}" autocomplete="email"
                                        required>
                                </div>
                                <div class="modal-footer">
